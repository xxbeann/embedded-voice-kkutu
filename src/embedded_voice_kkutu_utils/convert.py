...
def dump_words(
    ...
    words_dict: dict[str, str] = None,
    path: str = ASSETS_WORDS_PATH,
):
    if words_list == None and words_dict == None:
        raise ValueError("Least one of words_list or words_dict should be provided.")
    _dumping = {
        "enabled": {"words_list": words_list != None, "words_dict": words_dict != None},
    }
    if words_list != None:
...
def main() -> int:
    ...
    parser.add_argument(
        "--in-file",
        type=str,
        default=f"{KKUTU_SUBMODULE_PATH}/db.sql",
        help="Input file path. Default is `db.sql` in kkutu submodule. (./kkutu/db.sql)",
    )
    parser.add_argument(
        "--out-file",
        type=str,
        default=ASSETS_WORDS_PATH,
        help="Output file path. Default is `./assets/words.json`",
    )
    parser.add_argument(
        "--in-type",
        choices=["kkutu"],
        default="kkutu",
        help="Input db type. Default is `kkutu`",
    )
